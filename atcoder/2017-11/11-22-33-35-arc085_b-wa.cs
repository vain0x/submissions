...
public sealed partial class Program
{
    ...
    private long ScoreCore(int picker, int i, int j)
    {
        if (j == n - 1)
        {
            return Math.Abs(ws[picker][i] - vs[n - 1]);
        }
    ...
    private long Score(int picker, int i, int j)
    {
    ...
    private long Solve()
    {
        if (n == 1)
        {
            return Math.Abs(w - vs[0]);
        }

        ws = 2.MakeArray(p => n.MakeArray(i => vs[i]));
        ws[X][0] = w;

        memo = 2.MakeArray(p => n.MakeArray(i => n.MakeArray(j => -1L)));

        return Enumerable.Range(1, n - 1).Select(j => Score(X, 0, j)).Max();
    }
