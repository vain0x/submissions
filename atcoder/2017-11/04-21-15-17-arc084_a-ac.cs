...
public sealed partial class Program
{
    ...
    private long Solve()
    {
        ...
        // xks[yi]: 中部品 yi を中部に差し込んだときに使用可能な上部部品の個数
        var xks = n.MakeArray(i => 0L);

        {
            var xr = 0;
            for (var yi = 0; yi < n; yi++)
            {
                while (xr < n && xs[xr] < ys[yi])
                {
                    xr++;
                }
    ...
    private void Read()
    {
        var a = _scanner;
        n = a.N();
        xs = n.MakeArray(i => a.L());
        ys = n.MakeArray(i => a.L());
        zs = n.MakeArray(i => a.L());
    }
