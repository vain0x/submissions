...
fn enu() -> Vec<i64> {
    ...
    {
        #[derive(PartialEq)]
        struct Snuke(pub i64);

        impl Eq for Snuke {}
        impl PartialOrd for Snuke {
            fn partial_cmp(&self, other: &Snuke) -> Option<Ordering> {
                let lr = R(self.0);
                let rr = R(other.0);

                (rr, other.0).partial_cmp(&(lr, self.0))
            }
        }
        impl Ord for Snuke {
            fn cmp(&self, other: &Snuke) -> Ordering {
                self.partial_cmp(other).unwrap()
            }
        }

        let mut v = v.into_iter().map(|n| Snuke(n)).vec();
        let mut heap = std::collections::BinaryHeap::from(v);

        let mut w = Vec::new();
        let mut pr = 0.0;
        let mut pn = 0;
        while let Some(Snuke(n)) = heap.pop() {
            if n < pn {
                continue;
            }
            w.push(n);
