...
public sealed class Solver
{
    ...
    Tuple<int, int>[] SolveCore(long[] xs, int maxIndex)
    {
        var n = xs.Length;
        ...
        while (i + 1 < n && instr.Count <= n * 2)
        {
            ...
            {
                i++;
                instr.Add(Tuple.Create(maxIndex, i));
                continue;
            }
    ...
    public Tuple<int, int>[] Solve(long[] xs)
    {
        var maxAbs = -1L;
        var index = 0;
    ...
    public void VerifyRandomly()
    {
        ...
        for (var n = 2; n <= 50; n++)
        {
            ...
            {
                var xs = n.MakeArray(i => (long)rng.Next(xl, xr));
                var instr = Solve(xs);

                if (instr.Length > n * 2)
                {
                    Console.WriteLine("Couldn't solve: " + xs.Intercalate(", "));
                }
            }
        }
    }
...
public sealed partial class Program
{
    ...
    public void EntryPoint()
    {
        ...
        var instr = solver.Solve(A.Select(x => (long)x).ToArray());
