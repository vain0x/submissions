    ...
    private IEnumerable<long> Solve()
    {
        ...
        for (var k = n - 1; k >= 1; k--)
        {
            ...
            for (var i = 0; i < m - 1; i++)
            {
                ...
                // i 番目の点でスタミナ消費を行わないことにより生じるロス (絶対値)
            ...
            scores[k] = scores[k + 1] - minLoss;

            // points を更新する。
            {
                var p = points[minIndex];
                var q = points[minIndex + 1];
                var y = Math.Min(x, p.y + q.y);
                points[minIndex] = new { t = q.t, y };
                for (var i = minIndex + 1; i < m - 1; i++)
                {
                    points[i] = points[i + 1];
                }
