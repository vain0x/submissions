...
public sealed partial class Program
{
    ...
    private bool Solve()
    {
        var counts = S.Split('T').Select(fs => fs.Length).ToArray();
        X -= counts[0];
        if (counts.Length == 1)
        {
            return X == 0 && Y == 0;
        ...
        var vs = counts.Skip(1).ToArray();

        var n = (vs.Length + 1) / 2;

        var dxs = n.MakeArray(i => i * 2 + 1 < vs.Length ? vs[i * 2 + 1] : 0).OrderByDescending(dx => dx);
        var dys = n.MakeArray(i => vs[i * 2]).OrderByDescending(dy => dy);
        var dps = dxs.Zip(dys, (dx, dy) => Tuple.Create(dx, dy)).ToArray();
...

        {
            ...
            var dx = dps.Last().Item1;
            var dy = dps.Last().Item2;
