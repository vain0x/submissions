...
{
    public static X[] MakeArray<X>(this int count, Func<int, X> func = null)
    {
        var xs = new X[count];
        if (func != null)
        {
...
public sealed partial class Program
{
    ...
    private long[] Solve()
    {
        ...
        var dp = N.MakeArray(i => (N + 1).MakeArray<long>());
        
        // dp[_][0] = 0 (スタミナ未消費時は知力 0)
        // dp[_][1] = X (最初のスタミナ消費後は知力 X。)
        for (var i = 0; i < N; i++)
        {
            dp[i][0] = 0;
            dp[i][1] = X;
