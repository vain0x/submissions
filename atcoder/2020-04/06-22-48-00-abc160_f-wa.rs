...
>(
    ...
    for i in 0..node_count {
        ...
        let mut accum = identity.clone();
        let mut accums_from_tail = vec![identity.clone(); deg];

        for j in (1..deg).rev() {
            accums_from_tail[j - 1] = operate(dp[node][j].clone(), accums_from_tail[j].clone());
        }
...
fn main() {
    ...
    let identity = X::empty();

    let operate = |acc: X, x: X| -> X {
        X::append(
            acc,
            X {
                combo: x.combo * fact_inv[x.size] % P,
                size: x.size,
            },
        )
    };
