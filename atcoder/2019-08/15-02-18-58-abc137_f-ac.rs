...
pub fn pow(x: i64, n: i64, P: i64) -> i64 {
    ...
    // P^4 < 10^18 だから余りを取るのは2ステップに1回でいい
    'a: loop {
        for _ in 0..2 {
            if n <= 0 {
                break 'a;
...
fn main() {
    ...
    for i in 0..Pz {
        ...
        for k in 0..Pz {
            ...
            b *= pow((Pz - i) as i64, (Pz - 1 - k) as i64, P);
            // |b|≤10^8 の増減を 10^8 回行なってもオーバーフローしないので余りは取らなくていいはず
            B[k] -= b;
        }
