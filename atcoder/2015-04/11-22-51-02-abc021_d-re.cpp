...
template<ull Max = 30*30> ull combi(int n, int r) { if ( n < 0 || r < 0 ) { return 0; } if (!(n * r * (n - r))) { return 1; } assert(n <= Max && r <= Max); static ull t[Max+1][Max+1] {}; return t[n][r] ? t[n][r] : (t[n][r] = (combi<Max>(n - 1, r) + combi<Max>(n - 1, r - 1)) % divisor); }
static ull benchmark(std::function<void()> f) { ull start = clock(); f(); ull end = clock(); return (end - start); }

void mymain()
{
	int n, k;
	cin >> n >> k;
	cout << (combi<21000>(n + k - 1, k)) << endl;
}
