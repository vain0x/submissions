...
// -----------------------------------------------
// No free memory manager
// -----------------------------------------------

void *nfh_alloc(size_t size, size_t h) {
  struct Block {
    char *next;
    size_t size;
    size_t level;
  ...
  static struct Block s_blocks[10 + 1];

  if ((size - 1) > 0x2000 || h > 10) {
    return calloc(size, 1 << h);
  }
  ...
  struct Block *block = &s_blocks[h];
  if (block->size < size) {
    while (block->size < size) {
      block->level++;
      block->size = (1 << h) * (1 << block->level);
    }

    size_t align = 1 << h;
    size_t count = block->size >> h;
    assert(count * align == block->size);

    block->next = calloc(count, align);
  }

  assert(block->size >= size);
  void *addr = block->next;
  block->next += size;
  block->size -= size;
  return addr;
}

void *nfh_calloc(size_t count, size_t size) {
  size_t h = 0;
  while (size > (1 << h)) {
    h++;
  }
  ...
  size_t align = 1 << h;
  size_t total = (count * size + align - 1) & -align;
  return nfh_alloc(total, h);
}

void *nfh_malloc(size_t size) {
  return nfh_calloc(1, size);
}

void nfh_realloc(void) {
  // FIXME: unimpl
}

void nfh_free(void *_addr) {
  // pass.
}

#define malloc nfh_malloc
#define calloc nfh_calloc
#define realloc nfh_realloc
#define free nfh_free
