...
struct Graph {
    ...
    state: Vec<State>,
    cycles: Vec<(usize, usize, usize)>,
}

impl Graph {
    fn len(&self, xs: usize) -> usize {
        self.link[xs].2
    }
    ...
    fn to_vec(&self, mut xs: usize, ys: usize) -> Vec<usize> {
        let mut vec = vec![];
        while xs != ys {
    ...
    fn cycle_len(&self, up: usize, vp: usize) -> usize {
        self.len(up) - self.len(vp) + 1
    }

    fn is_ok(&self, up: usize, vp: usize) -> bool {
        let N = self.G.len();

        let vs = self.to_vec(up, vp);
        let set = vs.iter().collect::<BTreeSet<_>>();
    ...
    fn solve(mut self) -> Option<Vec<usize>> {
        ...
        self.cycles.sort();
        for &(_, up, vp) in &self.cycles {
            if self.is_ok(up, vp) {
                return Some(self.to_vec(up, vp));
            }
        }
        None
    }
