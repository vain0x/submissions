...
pub fn main() {
    let S = rl();
    let A = S.chars().map(|c| (c as i32 - '0' as i32) as i64).vec();

    fn dfs(A: &[i64], i: usize, s: i64, l: i64) -> i64 {
        if i == A.len() {
            l + s
        } else {
            let plus = dfs(A, i + 1, s + l, A[i]);
            let cons = dfs(A, i + 1, s, l * 10 + A[i]);
            plus + cons
        }
    };

    let sum = dfs(&A, 1, 0, A[0]);
    println!("{}", sum);
    return;
}
