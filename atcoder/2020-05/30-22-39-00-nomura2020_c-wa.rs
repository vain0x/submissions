...
fn main() {
    ...
    if (0..=N).any(|d| A[d] > (1 << d)) {
        ...
        // 内部ノードの個数の最小値。これを下回ると子世代への経路を構築できない。
        let mut lb = vec![0; N + 1];
        for d in (0..N).rev() {
            lb[d] = (A[d + 1] + lb[d + 1] + 1) / 2;
        }

        #[cfg(debug_assertions)]
        eprintln!("\x1B[33m{}\x1B[0m = {:?}", "lb", lb);

        // 内部ノードの個数の最大値。これを上回ると親世代が足りない。
        let mut ub = vec![0; N + 1];
        for d in (0..N).rev() {
            ub[d] = (A[d + 1] + ub[d + 1]).min((1 << d) - A[d]);
            ok = ok && lb[d] <= ub[d];
        }

        #[cfg(debug_assertions)]
        eprintln!("\x1B[33m{}\x1B[0m = {:?}", "ub", ub);

        let mut b = ub.clone();
        for d in 1..=N {
            b[d] = b[d].min((b[d - 1] * 2).saturating_sub(A[d]));
        }
