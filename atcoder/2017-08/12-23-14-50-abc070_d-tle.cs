...
{
    void DfsCore(int u, IReadOnlyList<IReadOnlyList<Edge>> graph, long cost, bool[] done, long[] dist)
    {
        done[u] = true;
        dist[u] = cost;
        ...
        foreach (var e in graph[u])
        {
            ...
            DfsCore(e.V, graph, cost + e.Cost, done, dist);
        }
    }

    long[] Dfs(IReadOnlyList<IReadOnlyList<Edge>> graph, int u)
    {
        var dist = graph.Count.MakeArray(_ => long.MaxValue);
        DfsCore(u, graph, 0L, new bool[graph.Count], dist);
        return dist;
    }
    ...
    public void EntryPoint()
    {
        var a = scanner;
        var n = a.N();
        var es = (n - 1).MakeArray(_ => new Edge(a.N() - 1, a.N() - 1, a.L()));
        var q = a.N();
        var k = a.N() - 1;

        var graph = n.MakeArray(_ => new List<Edge>());
        foreach (var e in es)
        {
            graph[e.U].Add(e);
            graph[e.V].Add(new Edge(e.V, e.U, e.Cost));
        }

        var dist = Dfs(graph, k);
