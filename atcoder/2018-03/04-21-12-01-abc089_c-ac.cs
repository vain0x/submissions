...
public sealed partial class Program
{
    ...
    public void EntryPoint()
    {
        var I = _scanner;
        var N = I.N();
        var ls = N.MakeArray(_ => _input.ReadLine()).Where(n => "MARCH".Contains(n[0])).GroupBy(n => n[0]).Select(g => (long)g.Count()).ToArray();
        Array.Copy(ls, ks, ls.Length);
